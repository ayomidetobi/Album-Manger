...
const AlbumSearchSortLogic = () => {
  const { albums: initialAlbums, isLoading: isInitialLoading, isError } = useAlbums(); // Fetch albums using useAlbums hook
  ...
  if (isInitialLoading) {
    return { albums: [], isLoading: true, query, sortedField, sortOrder, handleSearch: handleSearchDebounced, handleSort };
  }

  if (isError) {
    console.error('Error loading albums.');
    return { albums: [], isLoading: false, query, sortedField, sortOrder, handleSearch: handleSearchDebounced, handleSort };
  }
